...
for resolution in "${resolutions[@]}"; do
  ...
  for input_file in "$input_dir"/*_grounded.exr; do
    ...
    output_file="$output_dir"/"${base_name}_brdf_rohs_${resolution}_t5_p30.exr"

    # Measure the Wave Optics BRDF model
    ./gen_brdf -m Wave -d ROHS -i "$input_file" -o "$output_file" -w 0.625 -p 10.0 -r "$resolution" -z -s 5 -t 30
