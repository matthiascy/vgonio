//! Measurement file I/O: the VGMO codec and output-format dispatch.
//!
//! Moved here from `vgonio-app::io` (DIST Task 2.5). `vgonio-app` is downstream
//! of `vgonio-measurement`, so the VGMO codec (which the moved `Measurement`
//! read/write paths call) and the output-format dispatch must live in this
//! crate to avoid a dependency cycle. `vgonio-app/src/io.rs` is now a thin
//! shim that re-exports `vgn_measurement::io::*`.
//!
...
pub mod vgmo {
    ...
    use vgn_core::{
        ...
        BrdfLevel, MeasurementKind, Version,
    };
...
pub fn write_measured_data_to_file(
    ...
    for (i, measurement) in data.iter().enumerate() {
        ...
        cli_note!(Indent::DETAIL, "Saving to \"{}\"", filepath.display());
...
pub fn write_single_measured_data_to_file(
    ...
    cli_step!(Indent::SUBSECTION, "Saving measurement data...");
    cli_note!(Indent::DETAIL, "Saving as \"{}\"", filepath.display());
