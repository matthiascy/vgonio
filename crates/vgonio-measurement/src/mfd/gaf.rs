//! GAF measured-data type + the backend-dispatch shim. The wgpu rendering that
//! produces this data lives in `vgonio-measurement-wgpu`.

use vgn_bxdf::impl_any_measured_trait;
use vgn_core::{res::Handle, units::Radians, utils::range::StepRangeIncl, MeasurementKind};

use crate::{
    backend::GafBackend,
    cache::ComputeCache,
    measurement::{AnyMeasured, Measurement},
...
/// Delegates to the registered GAF backend (the wgpu impl lives in
/// `vgonio-measurement-wgpu`). Phase 1 had this inline.
pub fn measure_masking_shadowing_function(
    params: GafMeasurementParams,
    surfaces: &[Handle],
    cache: &ComputeCache,
    backend: &dyn GafBackend,
) -> Box<[Measurement]> {
    backend.measure_masking_shadowing_function(params, surfaces, cache)
}
