...
#[cfg(feature = "io")]
use std::path::Path;
use vgonio_core::bxdf::{BrdfParamKind, BrdfProxy, MeasuredBrdfKind, OutgoingDirs, ProxySource};
...
impl VgonioBrdf {
    ...
    pub fn write_as_exr(
        ...
        resolution: u32,
    ) -> Result<(), vgonio_core::error::VgonioError> {
        use exr::prelude::*;
        let n_wi = self.params.incoming.len();
        let n_spectrum = self.spectrum.len();
        ...
        for (l, snapshot) in self.snapshots().enumerate() {
            ...
            let layer_attrib = LayerAttributes {
                ...
                capture_date: Text::new_or_none(vgonio_core::utils::iso_timestamp_from_datetime(
                    timestamp,
                )),
        ...
        image.write().to_file(filepath).map_err(|err| {
            vgonio_core::error::VgonioError::new(
                format!(
...
impl AnyMeasuredBrdf for VgonioBrdf {
    vgonio_core::any_measured_brdf_trait_common_impl!(VgonioBrdf, Vgonio);
