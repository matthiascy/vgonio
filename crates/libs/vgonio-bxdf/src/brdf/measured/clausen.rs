...
use crate::brdf::measured::{BrdfParam, MeasuredBrdf, Origin};
use std::{
    borrow::Cow,
    f32,
    fs::File,
...
impl AnyMeasuredBrdf for ClausenBrdf {
    vgonio_core::any_measured_brdf_trait_common_impl!(ClausenBrdf, Clausen);
    ...
    fn proxy(&self, iors: &IorReg) -> BrdfProxy {
        ...
        BrdfProxy::new(
            ...
            self,
            Cow::Borrowed(&self.params.i_thetas),
            Cow::Borrowed(&self.params.phis),
            o_dirs,
            Cow::Owned(resampled),
            Cow::Owned(iors_i),
            Cow::Owned(iors_t),
