...
use vgonio_core::{
    bxdf::{AnalyticalBrdf, BrdfProxy, MicrofacetDistroKind, OutgoingDirs},
    math::Sph2,
    optics::Ior,
    units::{rad, Radians},
...
impl<'a> NllsqBrdfFittingProxy<'a, { Symmetry::Isotropic }> {
    ...
    fn jacobian(&self) -> Matrix<f64, Dyn, U1, Owned<f64, Dyn, U1>> {
        let mut jacobian =
            Matrix::<f64, Dyn, U1, Owned<f64, Dyn, U1>>::zeros(self.proxy.samples().len());
        let strides = self.proxy.samples().strides();
