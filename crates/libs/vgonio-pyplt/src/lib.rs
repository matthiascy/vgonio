use exr::prelude::{FlatImage, FlatSamples, Text};
...
pub fn load_py_src_code(filename: &str) -> Result<CString, Box<dyn std::error::Error>> {
    let py_src_dir = get_py_src_dir()?;
    let path = py_src_dir.join(filename);
            ...
            brdf.iter()
                .map(|&brdf| {
                    ...
                    let (slices_phi, slices_phi_opp): (Vec<Vec<_>>, Vec<Vec<_>>) = theta_i
                        .iter()
                        .map(|&theta_i| {
                            let wi = Sph2::new(rad!(theta_i), phi);
...
impl BrdfFittingPlotter {
    ...
    pub fn plot_non_interactive(
        ...
        let src_code = load_py_src_code("brdf_fitting_non_interactive.py")?;
...
mod tests {
    #[test]
    fn test_py_src_dir() {
        let py_src_dir = super::get_py_src_dir().unwrap();
        assert!(py_src_dir.exists());
    }
}
