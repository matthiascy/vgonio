...
extern "C" __global__ void difference(
...
extern "C" __global__ void difference_sqr(
    const float* __restrict__ xs,
    const float* __restrict__ ys,
    float* __restrict__ result,
    size_t n)
...
// Element-wise squared difference of two arrays if the arrays are 16-byte aligned
extern "C" __global__ void difference_sqr_vec4(
    const float4* __restrict__ xs,
    const float4* __restrict__ ys,
    float4* __restrict__ result,
    size_t n_vec4)
{
    for (size_t tid = global_tid(); tid < n_vec4; tid += gridDim.x * blockDim.x) {
        const float4 a = xs[tid];
        const float4 b = ys[tid];
        float4 r;
        float d;

        d = a.x - b.x; r.x = __fmaf_rn(d, d, 0.0f);
        d = a.y - b.y; r.y = __fmaf_rn(d, d, 0.0f);
        d = a.z - b.z; r.z = __fmaf_rn(d, d, 0.0f);
        d = a.w - b.w; r.w = __fmaf_rn(d, d, 0.0f);
...
//    Else set shift=-1 and mask=weight_stride (for division); OR pre-expand weights.
extern "C" __global__ void difference_sqr_lncos(
    const float* __restrict__ xs,
    const float* __restrict__ ys,
    const float* __restrict__ weights,
    float* __restrict result,
    const unsigned int weight_stride,
    size_t n)
{
    ...
    for (size_t tid = global_tid(); tid < n; tid += gridDim.x * blockDim.x) {
        int widx = (shift >= 0) ? (int)(tid >> shift) : (int)(tid / weight_stride);
