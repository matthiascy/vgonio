...
rust::Vec<float> brdf_wavelengths(const BRDF &brdf) {
    ...
    std::copy(std::begin(brdf.wavelengths()), std::end(brdf.wavelengths()), std::back_inserter(result));
    return result;
}
...
rust::Vec<float> brdf_eval(const BRDF &brdf, float theta_i, float phi_i, float theta_o, float phi_o) {
    ...
    Vector3f wi = Vector3f(
        std::cos(phi_i) * std::sin(theta_i),
        std::sin(phi_i) * std::sin(theta_i),
        std::cos(theta_i)
    );
    Vector3f wo = Vector3f(
        std::cos(phi_o) * std::sin(theta_o),
        std::sin(phi_o) * std::sin(theta_o),
        std::cos(theta_o)
    );
    float cos_theta_o = std::cos(theta_o);
    Spectrum s = brdf.eval(wi, wo);
    if (cos_theta_o <= 1e-6f)
        s = Spectrum(0.f, s.size());
