        ...
        Python::with_gil(|py| {
            ...
            let (n_theta_o, o_thetas, samples, samples_opp) = match brdf.kind() {
                ...
                MeasuredBrdfKind::Vgonio => {
                    ...
                    unsafe {
                        let sampled = sampler.sample_slice_at(wi, phi_o).unwrap();
                        let sampled_opp = sampler.sample_slice_at(wi, phi_o_opp).unwrap();
                        assert_eq!(sampled.len(), n_theta_o * n_spectrum);
                        for i in 0..o_thetas.len() {
                            samples.as_slice_mut().unwrap()[i] = sampled[i * n_spectrum] as f64;
                            samples_opp.as_slice_mut().unwrap()[i] =
                                sampled_opp[i * n_spectrum] as f64;
                        }
