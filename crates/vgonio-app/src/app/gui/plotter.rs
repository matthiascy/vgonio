...
use uuid::Uuid;
use vgn_bxdf::distro::{BeckmannDistribution, MicrofacetDistribution, TrowbridgeReitzDistribution};
#[cfg(feature = "fitting")]
...
use vgn_uxgx::gui::widgets::{AngleKnob, AngleKnobWinding};
...
impl Curve {
    ...
    /// # Arguments
    ///
    /// * `first_part` - The first part of the data, which is the measured data for the selected
    ///   azimuthal angle.
    /// * `opposite` - The second part of the data, which is the measured data for the opposite
    ///   azimuthal angle. If `None`, the curve is created from the first part only.
