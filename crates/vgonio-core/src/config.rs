//! Configuration for Vgonio.
use crate::{error::VgonioError, TriangulationPattern};
...
pub struct Config {
    ...
    pub sys_cache_dir: PathBuf,

    /// Path to the data files.
    pub sys_data_dir: PathBuf,
    ...
    pub fn load_config(filepath: Option<&Path>) -> Result<Self, VgonioError> {
        ...
        let sys_cache_dir = {
            let mut cache_dir = dirs::cache_dir()
                .ok_or(VgonioError::new("System cache directory not found.", None))?;
        ...
        match output_dir {
            Some(dir) => {
                ...
                if resolved.is_none() {
                    ...
                    Err(VgonioError::new(
                        format!("Output directory {} is not a directory", dir.display()),
                        None,
                    ))
                } else {
                    Ok(resolved.unwrap())
