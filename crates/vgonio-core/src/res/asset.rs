...
/// Allocator for unique asset type identifiers.
pub struct AssetTypeIdAllocator;

impl AssetTypeIdAllocator {
    /// Generates the next unique asset type identifier.
...
/// Macro to implement the `Asset` trait for a given type with a specified name.
#[macro_export]
macro_rules! asset {
    ($t:ty, $n:expr) => {
