...
impl MicroSurface {
    ...
    pub fn read_from_wavefront(
        ...
        let mut reader = BufReader::new(file);
        surf::read_wavefront(&mut reader, filepath, axis, unit)
    }
    ...
    pub fn write_to_file(
        ...
        use surf::vgms::VgmsHeaderExt;

        let timestamp = {
            let mut timestamp = [0_u8; 32];
            timestamp.copy_from_slice(vgn_core::utils::iso_timestamp().as_bytes());
            timestamp
        };
        let header = Header {
            meta: HeaderMeta {
        ...
        surf::vgms::write(&mut writer, header, &self.samples).map_err(|err| {
