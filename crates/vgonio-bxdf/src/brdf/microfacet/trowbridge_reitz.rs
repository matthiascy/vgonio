...
impl Bxdf for TrowbridgeReitzBrdf {
    ...
    fn pds(&self, wis: &[Vec3], wos: &[Vec3], ior_i: &Ior, ior_t: &Ior) -> Box<[f64]> {
        ...
        for i in 0..wis.len() {
            ...
            for j in 0..wos.len() {
                let wo = wos[j];
                let pd = self.pd(&wi, &wo, ior_i, ior_t);
                result[i * wos.len() * 2 + j * 2].write(pd[0]);
                result[i * wos.len() * 2 + j * 2 + 1].write(pd[1]);
            }
        }
    ...
    fn pds_iso(&self, wis: &[Vec3], wos: &[Vec3], ior_i: &Ior, ior_t: &Ior) -> Box<[f64]> {
        ...
        for i in 0..wis.len() {
            ...
            for j in 0..wos.len() {
                let wo = wos[j];
                result[i * wos.len() + j].write(self.pd_iso(&wi, &wo, ior_i, ior_t));
            }
        }
        unsafe { result.assume_init() }
    }
