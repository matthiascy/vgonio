...
/// Parametrisation of the RGL BRDF.
///
...
impl BrdfParam for RglBrdfParametrisation {
    fn kind() -> BrdfParamKind { BrdfParamKind::InOutDirs }
}

impl RglBrdfParametrisation {
...
impl RglBrdf {
    ...
    pub fn load(path: &Path, medium: MediumId) -> Self {
        ...
        let parameterisation = RglBrdfParametrisation {
            n_zenith_i,
            n_zenith_o,
            incoming,
            outgoing,
...
impl<'a> Iterator for BrdfSnapshotIterator<'a, RglBrdfParametrisation, 3> {
...
impl AnyMeasuredBrdf for RglBrdf {
    ...
    fn proxy(&self, iors: &IorReg) -> BrdfProxy {
        log::debug!(
            "Creating proxy for RGL BRDF, parametrisation: {:?}",
            self.params
        );
        let iors_i = iors
