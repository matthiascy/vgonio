use base::math::{rcp_f32, rcp_f64, sqr, Sph2, Vec3};
use std::intrinsics::sqrtf64;

use crate::{
    dist::TrowbridgeReitzDistribution, impl_common_methods, MicrofacetBasedBrdfModel,
...
impl MicrofacetBasedBrdfModelFittingModel for TrowbridgeReitzBrdfModel {
    fn partial_derivatives(&self, wis: &[Vec3], wos: &[Vec3]) -> Box<[f64]> {
        debug_assert!(
            wis.len() == wos.len(),
            "incident and outgoing directions have different length"
        );
        let mut result = Box::new_uninit_slice(wi.len() * 2);
        for i in 0..wis.len() {
            let wi = wis[i];
            let wo = wos[i];
            debug_assert!(wi.is_normalized(), "incident direction is not normalized");
            ...
            let a = 1.0
                + (cos_phi_h2 * rcp_f64(alpha_x2) + sin_phi_h2 * rcp_f64(alpha_y2)) * tan_theta_h2;

            let phi_h = wh.y.atan2(wh.x);
            let phi_i = wi.y.atan2(wi.x);
            let phi_hi = (phi_h - phi_i).abs();
            let cos_phi_hi = phi_hi.cos();
            let sin_phi_hi = phi_hi.sin();
            let cos_theta_hi = wi.dot(wh);
            let tan_theta_hi2 = (1.0 - cos_theta_hi * cos_theta_hi) / (cos_theta_hi * cos_theta_hi);
            let b =
                (1 + alpha_x2 * cos_phi_hi * tan_theta_hi2 + alpha_y2 * sin_phi_hi * tan_theta_hi2)
                    .sqrt();
            ...
            let cos_theta_ho = wo.dot(wh);
            let tan_theta_ho2 = (1.0 - cos_theta_ho * cos_theta_ho) / (cos_theta_ho * cos_theta_ho);
            let c =
                (1 + alpha_x2 * cos_phi_ho * tan_theta_ho2 + alpha_y2 * sin_phi_ho * tan_theta_ho2)
                    .sqrt();
            ...
            let common_denominator = std::f64::consts::PI * a * a * a * one_plus_b_one_plus_c_sqr;
            ...
            let nominator_x = f
                ...
                * (-cos_phi_ho * a * one_plus_b * tan_theta_ho2_over_c
                    ...
                    + 4 * cos_phi_h2 * tan_theta_h2 * one_plus_b_one_plus_c * rcp_f64(alpha_x4)
            ...
            let nominator_y = f
                ...
                * (-sin_phi_ho * a * one_plus_b * tan_theta_ho2_over_c
                    ...
                    + 4 * sin_phi_h2 * tan_theta_h2 * one_plus_b_one_plus_c * rcp_f64(alpha_y4)
            ...
            result[i * 2].write(dfr_dalpha_x);
            result[i * 2 + 1].write(dfr_dalpha_y);
        }
