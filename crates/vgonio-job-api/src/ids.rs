...
//!   `"al"`, ...). Backed by a leak-pooled `Box<str>` intern table for the process lifetime, *not*
//!   a UUID; content equality is the key invariant so that `MediumId::AIR` from two crates compare
...
//!   the *built-in* `MediumId` constants (`VACUUM`, `AIR`, `AL`, ...); custom media added at
//! runtime   require either shipping the corresponding `.ior.ron` blob to the worker as an input
...
//!
//! - **`Checksum`** (defined in [`crate::artifact`]) is a content-derived integrity tag (sha256).
...
//!   publishes the result as a single .vgbsdf zip with per-bounce-level subdirs
...
uuid_id!(
    ...
    "Identifies a single artifact blob (job input or output). Carried inside \
     ...
     `.vgbsdf`: JAR-style zip container (`Store` method, **no container compression**, since the \
     EXR payloads inside are already compressed), the canonical archival format for \
     ...
     heightfield's `Checksum`). \n- `.vgms` / `.vgmo`: bespoke local cache files for heightfields \
     and measurement scratch, opt-in LZ4-compressible via `CompressionScheme::Lz4` (cache layer \
     only; never crosses the artifact-store boundary as LZ4, workers exchange the uncompressed \
...
/// crates: each `vgonio-measurement-{embree,cuda,wgpu}` (and OptiX, folded in or split later)
