...
pub struct GridRT<'hf> {
    ...
    msurf: &'hf MicroSurface,

    /// Corresponding `TriangleMesh` of the surface.
    msurf_mesh: &'hf MicroSurfaceMesh,
...
impl<'ms> GridRT<'ms> {
    ...
    pub fn world_to_grid_3d(&self, world_pos: Vec3) -> IVec2 {
        ...
        let (x, y) = match self.msurf_mesh.alignment {
        ...
        // TODO:
        IVec2::new(
            ((x - self.origin.x) / self.msurf.du) as i32,
            ((y - self.origin.y) / self.msurf.dv) as i32,
        )
    }
    ...
    pub fn world_to_grid_2d(&self, world_pos: Vec2) -> IVec2 {
        IVec2::new(
            ((world_pos.x - self.origin.x) / self.msurf.du) as i32,
            ((world_pos.y - self.origin.y) / self.msurf.dv) as i32,
        )
    }
    ...
    fn triangles_at(&self, pos: IVec2) -> [(u32, [Vec3; 3]); 2] {
        ...
        let cell = (self.msurf.cols - 1) * pos.y as usize + pos.x as usize;
        let pts = &self.msurf_mesh.facets[cell * 6..cell * 6 + 6];
        log::debug!(
            "             - cell: {:?}, tris: {:?}, pts {:?}",
            cell,
            [cell * 2, cell * 2 + 1],
        ...
        [
            (
                (cell * 2) as u32,
                [
                    self.msurf_mesh.verts[pts[0] as usize],
                    self.msurf_mesh.verts[pts[1] as usize],
                    self.msurf_mesh.verts[pts[2] as usize],
                ],
            ),
            (
                (cell * 2 + 1) as u32,
                [
                    self.msurf_mesh.verts[pts[3] as usize],
                    self.msurf_mesh.verts[pts[4] as usize],
                    self.msurf_mesh.verts[pts[5] as usize],
                ],
            ),
        ]
    }
    ...
    fn altitudes_of_cell(&self, pos: IVec2) -> [f32; 4] {
        ...
        [
            self.msurf.sample_at(x, y).as_f32(),
            self.msurf.sample_at(x, y + 1).as_f32(),
            self.msurf.sample_at(x + 1, y + 1).as_f32(),
            self.msurf.sample_at(x + 1, y).as_f32(),
        ]
    }
    ...
    pub fn traverse(&self, ray_org_world: Vec2, ray_dir: Vec2) -> GridTraversal {
        ...
        if is_parallel_to_x_axis && !is_parallel_to_y_axis {
            ...
            let (dir, initial_dist, num_cells) = if ray_dir.x < 0.0 {
                (
                    -1,
                    ray_org_grid.x - ray_org_cell.x as f32 * self.msurf.du,
                    ray_org_cell.x - self.min.x + 1,
                )
            } else {
                (
                    1,
                    (ray_org_cell.x + 1) as f32 * self.msurf.du - ray_org_grid.x,
            ...
            for i in 0..num_cells {
                ...
                dists[i as usize + 1] = initial_dist + (i as f32 * self.msurf.du);
            }
            ...
            let (dir, initial_dist, num_cells) = if ray_dir.y < 0.0 {
                (
                    -1,
                    ray_org_grid.y - ray_org_cell.y as f32 * self.msurf.dv,
                    ray_org_cell.y - self.min.y + 1,
                )
            } else {
                (
                    1,
                    (ray_org_cell.y + 1) as f32 * self.msurf.dv - ray_org_grid.y,
            ...
            for i in 0..num_cells {
                ...
                dists[i as usize + 1] = initial_dist + (i as f32 * self.msurf.dv);
            }
            GridTraversal::Traversed { cells, dists }
        } else {
            ...
            let unit = Vec2::new(
                (1.0 + m * m).sqrt() * self.msurf.du,
                (1.0 + m_recip * m_recip).sqrt() * self.msurf.dv,
            )
            .abs();
            ...
            let mut accumulated = Vec2::new(
                if ray_dir.x < 0.0 {
                    ...
                    (ray_org_grid.x / self.msurf.du - curr_cell.x as f32) * unit.x
                } else {
                    //((curr_cell.x + 1) as f32 * self.surface.du - ray_org_grid.x) * unit.x
                    ((curr_cell.x + 1) as f32 - ray_org_grid.x / self.msurf.du) * unit.x
                },
                if ray_dir.y < 0.0 {
                    // (ray_org_grid.y - curr_cell.y as f32 * self.surface.dv) * unit.y
                    (ray_org_grid.y / self.msurf.dv - curr_cell.y as f32) * unit.y
                } else {
                    // ((curr_cell.y + 1) as f32 * self.surface.dv - ray_org_grid.y) * unit.y
                    ((curr_cell.y + 1) as f32 - ray_org_grid.y / self.msurf.dv) * unit.y
                },
            );
        ...
        tris.iter()
            .filter_map(|(index, pts)| {
                ...
                ray_tri_intersect_woop(ray, pts)
                    ...
                    .map(|isect| {
                        let tris_per_row = (self.msurf.cols - 1) * 2;
                        let cells_per_row = self.msurf.cols - 1;
                        ...
                        let is_last_row = cell_row == self.msurf.rows - 1;
                        let adjacent: Option<(usize, Vec3)> = if isect.u.abs() < 2.0 * f32::EPSILON
                        {
                            ...
                            if is_tri_index_odd || !is_first_col {
                                ...
                                Some((tri_index - 1, self.msurf_mesh.facet_normals[tri_index - 1]))
                            } else {
                                None
                            }
                            ...
                            if !is_tri_index_odd && !is_first_row {
                                ...
                                Some((
                                    tri_index - (tris_per_row - 1),
                                    (self.msurf_mesh.facet_normals[tri_index - (tris_per_row - 1)]),
                                ))
