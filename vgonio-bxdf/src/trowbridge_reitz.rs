...
impl MicrofacetDistributionModel for TrowbridgeReitzDistribution {
    ...
    fn eval_adf(&self, cos_theta: f64, cos_phi: f64) -> f64 {
        ...
        let e = match self.isotropy() {
            Isotropy::Isotropic => tan_theta2 * rcp_f64(alpha_xy),
            Isotropy::Anisotropic => {
                let cos_phi2 = sqr(cos_phi);
                let sin_phi2 = 1.0 - cos_phi2;
                tan_theta2 * (cos_phi2 / sqr(self.alpha_x) + sin_phi2 / sqr(self.alpha_y))
            }
        };
...
impl MicrofacetDistributionFittingModel for TrowbridgeReitzDistribution {
    fn adf_partial_derivatives(&self, cos_thetas: &[f64], cos_phis: &[f64]) -> Vec<f64> {
        ...
        match self.isotropy() {
            Isotropy::Isotropic => {
                debug_assert!(
                    (self.alpha_x - self.alpha_y).abs() < 1.0e-6,
                    "The alpha_x and alpha_y must be the same in the isotropic case."
                );
                let alpha2 = self.alpha_x * self.alpha_y;
                cos_thetas
                    .iter()
                    .map(|cos_theta| {
                        let cos_theta2 = sqr(*cos_theta);
                        let sec_theta2 = 1.0 / cos_theta2;
                        let tan_theta2 = if cos_theta2 == 0.0 {
                            f64::INFINITY
                        } else {
                            1.0 / cos_theta2 - cos_theta2
                        };
                        let numerator =
                            2.0 * self.alpha_x * (tan_theta2 - alpha2) * sqr(sec_theta2);
                        let denominator = std::f64::consts::PI * cube(alpha2 + tan_theta2);
